...
}};a(e).sortable(a.extend({axis:"x",distance:20,tolerance:"pointer",items:".ctabs-ctab",stop:function(o,p){g._stopSort();}}),d).disableSelection();this._nodes={body:f,head:j,headLeftWrapper:n,headLeft:k,headRightWrapper:l,headRight:i,headCenterWrapper:c,headCenter:e,adder:b};
},_initialFill:function(){var b=this;a.each(this.store,function(c){b._createCTab(c,undefined);});this._refresh();},_createCTab:function(k){var l=this;this.store[k].tab.hide();
this.store[k].panel.hide().appendTo(this._nodes.body);var h=a("<div>").addClass("ctabs-ctab ctabs-ctab-dimensions").addClass(this.options.cssFlex?"ctabs-ctab-flex":"ctabs-ctab-fix").attr("hash",k).insertBefore(this._nodes.adder);
...
}}this.store[b].panel.show();this.store[b].ctab.toggleClass("ctabs-ctab-active",true);this._workflow.currentHash=b;this._updateActive();if(a.isFunction(this.options.afterSelect)){this.options.afterSelect(b);
}},deselect:function(b){b=this._getHash(b);if(!this._workflow.currentHash||!this.store[this._workflow.currentHash]){return;}if(b&&this._workflow.currentHash!=b){return;
}if(a.isFunction(this.options.beforeDeselect)){if(this.options.beforeDeselect(this._workflow.currentHash)===false){return false;}}this.store[this._workflow.currentHash].panel.hide();
this.store[this._workflow.currentHash].ctab.toggleClass("ctabs-ctab-active",false);this._workflow.currentHash=null;this._updateActive();if(a.isFunction(this.options.afterDeselect)){this.options.afterDeselect(b);
}},title:function(b,c){b=this._getHash(b);if(!this.store[b]){return;}if(!c){return this.store[b].title.html();}this.store[b].title.html(c);},icon:function(c,b){c=this._getHash(c);
if(!this.store[c]){return;}if(!b){return this.store[c].icon;}if(a.isFunction(this.options.iconFormatter)){b=this.options.iconFormatter(c,this.store[c].icon,b);
